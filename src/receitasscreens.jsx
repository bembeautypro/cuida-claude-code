...
import React, { useState as useRC, useEffect as useEffRC } from 'react';
import { Screen, Button, IconButton, Avatar } from './ui.jsx';
import { IconArrowL, IconCamera, IconShare, IconInfo, IconCheck, IconPill, IconChevD, IconSearch } from './icons.jsx';
    ...
    <Screen hasTabBar={false}>
      ...
      {phase === 'idle' && (
        <div style={{ padding: '20px 20px 0' }}>
          ...
          <div style={{ marginTop: 24, padding: 16, borderRadius: 16, background: 'var(--c-surface)', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            <IconInfo size={18} style={{ color: 'var(--c-accent)', flexShrink: 0, marginTop: 1 }}/>
      ...
      {phase === 'done' && (
        <div style={{ padding: '20px 20px 0' }}>
          ...
          <Button variant="primary" full size="lg" onClick={() => go && go('receita-confirm')}>
            Revisar e confirmar
          </Button>
          <Button variant="ghost" full size="md" style={{ marginTop: 10 }} onClick={() => setPhase('idle')}>
            Fotografar de novo
          </Button>
        </div>
      )}
...
function ReceitaConfirmScreen({ go }) {
  ...
  return (
    <Screen hasTabBar={false}>
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <IconButton variant="plain" onClick={() => go && go('receitas')}><IconArrowL size={20}/></IconButton>
      ...
      <div style={{ padding: '12px 20px 0' }}>
        <div style={{ padding: 14, borderRadius: 16, background: 'var(--c-accent-soft)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <Avatar name="Dr. Ricardo Andrade" color="var(--c-accent)" fg="var(--c-accent-fg)" size={40}/>
          <div>
      ...
      <div style={{ padding: '16px 20px 0' }}>
            ...
            return (
              <div key={m.id} style={{
                ...
                <button onClick={() => setEditId(editing ? null : m.id)} style={{
                  ...
                  <IconChevD size={16} style={{ color: 'var(--c-text-muted)', transform: editing ? 'rotate(180deg)' : 'none', transition: 'transform .2s' }}/>
                </button>
        ...
        <div style={{ marginTop: 16, padding: 14, borderRadius: 14, background: 'var(--c-surface)', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
          <IconInfo size={16} style={{ color: 'var(--c-accent)', flexShrink: 0, marginTop: 2 }}/>
          <div style={{ fontSize: 12, color: 'var(--c-text-soft)', lineHeight: 1.5 }}>
            Ao confirmar, os remédios serão lançados na lista de medicamentos com os horários definidos acima.
          </div>
        </div>

        <Button variant="primary" full size="lg" style={{ marginTop: 18, background: saved ? 'var(--c-success)' : undefined }} onClick={handleConfirm}>
          {saved ? '✓ Lançado com sucesso!' : 'Confirmar e lançar no sistema'}
        </Button>
        <Button variant="ghost" full size="md" style={{ marginTop: 10 }} onClick={() => go && go('receitas')}>
          Cancelar
        </Button>
      </div>
    </Screen>
    ...
    <Screen hasTabBar={false}>
      ...
      {phase === 'idle' && (
        <div style={{ padding: '20px 20px 0' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 22 }}>
            {[
              [<IconCamera size={26}/>, 'Fotografar laudo', 'Aponte para o papel ou tela do resultado', 'var(--c-accent)', 'var(--c-accent-fg)'],
              [<IconShare size={24}/>, 'Upload PDF ou imagem', 'Arquivo salvo no celular ou nuvem', 'var(--c-surface)', 'var(--c-text)'],
      ...
      {(phase === 'results' || phase === 'saved') && (
        <div style={{ padding: '16px 20px 0' }}>
          ...
          <Button variant="primary" full size="lg" style={{ marginTop: 18, background: phase === 'saved' ? 'var(--c-success)' : undefined }} onClick={handleConfirm}>
...
export { ReceitasScreen, ReceitaConfirmScreen, LaudoOCRScreen };
