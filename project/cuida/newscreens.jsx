...
// ════════════════════════════════════════════════════════════
const ESP_ICONS = {
  'Cardiologia':          <window.IconHeart size={20}/>,
  ...
  'Endocrinologia':       <window.IconDroplet size={20}/>,
  default:               <window.IconCalendar size={20}/>,
};
...
function ProntuarioIcon({ section }) {
  const icons = {
    'Resumo de saúde':  <window.IconShield size={18}/>,
    'Histórico médico': <window.IconHeart size={18}/>,
    'Medicamentos':     <window.IconPill size={18}/>,
    ...
    'Consultas':        <window.IconCalendar size={18}/>,
  ...
  return (
    ...
    }}>{icons[section.title] || <window.IconInfo size={18}/>}</div>
  );
}
  ...
  return (
    <window.Screen>
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <window.IconButton variant="plain" onClick={() => go && go('home')}><window.IconArrowL size={20}/></window.IconButton>
        <window.IconButton variant="soft" size={36} onClick={() => go && go('consultas-cal')}><window.IconCalendar size={18}/></window.IconButton>
      </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
              ...
              <div style={{
                ...
                <window.IconCalendar size={18} style={{ color: next.fg, margin: '0 auto 4px' }}/>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 6, fontSize: 12, color: 'var(--c-text-soft)' }}>
                  <window.IconSearch size={12}/> {next.local}
                </div>
              </div>
              <window.IconChevR size={18} style={{ color: 'var(--c-text-muted)', flexShrink: 0, marginTop: 4 }}/>
            </div>

            {/* Actions */}
            <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
              <window.Button variant="outline" size="sm" style={{ flex: 1 }} onClick={() => go && go('consulta-detail')}>Detalhes</window.Button>
              <window.Button variant="primary" size="sm" style={{ flex: 1.4 }}
                    ...
                    return (
                      <button key={i} onClick={() => setPrepChecked(p => ({ ...p, [i]: !p[i] }))} style={{
                        ...
                        <div style={{
                          ...
                          {checked && <window.IconCheck size={12} stroke={2.5}/>}
                        </div>
                  ...
                  <div key={c.id} onClick={() => go && go('consulta-detail')} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 3, fontSize: 11, color: 'var(--c-text-muted)' }}>
                        <window.IconCalendar size={11}/> {c.date}
                      </div>
                    </div>
                    <span style={{
      ...
      <div style={{ padding: '20px 20px 0' }}>
        <window.Button variant="outline" full size="md" icon={<window.IconCalendar size={18}/>} onClick={() => go && go('add-consulta')}>
...
function MedsV2Screen({ go }) {
  ...
  return (
    <window.Screen>
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <window.IconButton variant="plain" onClick={() => go && go('home')}><window.IconArrowL size={20}/></window.IconButton>
        <window.IconButton variant="soft" size={36} onClick={() => go && go('add-med')}><window.IconPlus size={18}/></window.IconButton>
      </div>
      ...
      <div style={{ padding: '10px 20px 14px' }}>
        <window.SoftCard background="var(--c-accent)" padding="12px 16px" radius={16}
        ...
        <div style={{ padding: '0 20px 12px' }}>
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <window.Button variant="primary" size="sm" onClick={() => setLembretes(true)}>
      ...
      <div style={{ padding: '0 20px' }}>
        <button onClick={() => go && go('receitas')} style={{
          ...
          <window.IconChevR size={16} style={{ marginLeft: 'auto', color: 'var(--c-text-muted)' }}/>
        </button>
      </div>

      <div style={{ padding: '0 20px' }}>
        <window.Button variant="outline" full icon={<window.IconPlus size={18}/>} onClick={() => go && go('add-med')}>
...
function ExamesScreen({ go }) {
  ...
  return (
    <window.Screen>
            ...
            {pendentes.length > 0 && (
              <div style={{ marginBottom: 22 }}>
                <div className="cu-eyebrow" style={{ padding: '0 4px 10px', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <window.Dot color="var(--c-warn)" size={7}/>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                {realizados.map((e, i) => (
                  <div key={e.id} style={{
                    ...
                    <window.IconChevR size={16} style={{ color: 'var(--c-text-muted)', marginLeft: 4 }}/>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div style={{ padding: '20px 20px 0' }}>
        <window.Button variant="outline" full onClick={() => go && go('exames-full')} icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>}>
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 3, fontSize: 11, color: 'var(--c-text-muted)' }}>
          <window.IconCalendar size={11}/> {e.date}
      ...
      <window.IconChevR size={16} style={{ color: 'var(--c-text-muted)', marginLeft: 2 }}/>
    </div>
  );
}
...
function ProntuariosScreen({ go }) {
  ...
  const profiles = window.CUIDA_DATA.PROFILES;
  ...
  return (
    <window.Screen>
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <window.IconButton variant="plain" onClick={() => go && go('home')}><window.IconArrowL size={20}/></window.IconButton>
        <window.IconButton variant="soft" size={36}><window.IconSearch size={18}/></window.IconButton>
      </div>
      ...
      <div style={{ padding: '14px 20px 0' }}>
        <button onClick={() => setPersonOpen(p => !p)} style={{
          ...
          <window.Avatar name={person.name} color={person.color} fg={person.fg} size={40}/>
          ...
          <window.IconChevD size={18} style={{
          ...
          <div style={{
            ...
            {profiles.map(p => (
              <button key={p.id} onClick={() => { setActivePerson(p.id); setPersonOpen(false); }} style={{
                ...
                <window.Avatar name={p.name} color={p.color} fg={p.fg} size={34}/>
                <div style={{ flex: 1, textAlign: 'left' }}>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>{p.name}</div>
                  <div style={{ fontSize: 11, color: 'var(--c-text-soft)' }}>{p.relation} · {p.age} anos</div>
                </div>
                {p.id === activePerson && <window.IconCheck size={16} style={{ color: 'var(--c-success)' }}/>}
              </button>
            ))}
          </div>
        )}
        ...
        {!personOpen && (
          <div style={{ marginTop: 12, padding: '10px 14px', background: 'var(--c-surface)', borderRadius: 14, display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ flex: 1 }}>
              ...
              <window.Bar value={completeness} max={100} color={completeness === 100 ? 'var(--c-success)' : 'var(--c-accent)'} height={5}/>
            </div>
            {completeness < 100 && (
      ...
      <div style={{ padding: '16px 20px 0', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {PRONTUARIO_SECTIONS.map((s, i) => (
          <div key={s.id} onClick={() => { const r = {'Resumo de saúde':'resumo-saude','Histórico médico':'historico','Medicamentos':'meds-v2','Exames':'exames-full','Consultas':'consultas','Vacinas':'vacinas'}; go && go(r[s.title] || 'prontuarios'); }} style={{
            ...
            <window.IconChevR size={16} style={{ color: 'var(--c-text-muted)', flexShrink: 0 }}/>
          </div>
        ))}
      </div>
    </window.Screen>
