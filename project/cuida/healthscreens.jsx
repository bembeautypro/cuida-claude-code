...
function MedDetailScreen({ go, medId = 'm1' }) {
  const meds = window.CUIDA_DATA.MEDS_MARIA;
  const med = meds.find(m => m.id === medId) || meds[0];
  const [times, setTimes] = useSH(['07:00', '13:00']);
  const [receita, setReceita] = useSH(true); // simulated: has prescription
  ...
  return (
    <window.Screen>
      ...
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <window.IconButton variant="soft" size={36}><window.IconMoreV size={18}/></window.IconButton>
      </div>
      ...
      <div style={{ padding: '10px 20px 0' }}>
        <div style={{
          ...
          }}><window.IconCamera size={18}/></button>
        </div>
      </div>
      ...
      <div style={{ padding: '14px 20px 0' }}>
        ...
        <div style={{ marginTop: 20 }}>
          ...
          {receita ? (
            ...
            <button onClick={() => setReceita(true)} style={{
              ...
              <window.IconCamera size={20} style={{ color: 'var(--c-accent)' }}/>
          ...
          {showReceita && (
            <div style={{
              ...
              <div style={{ fontSize: 13, lineHeight: 1.6, color: 'var(--c-text-soft)', fontFamily: 'ui-monospace, Menlo, monospace', fontSize: 11 }}>
        ...
        <div style={{ marginTop: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <button style={{
              ...
              <window.IconPlus size={16}/> Adicionar orientação
            </button>
          </div>
        </div>
        ...
        <div style={{ marginTop: 20, padding: 16, borderRadius: 16, background: 'var(--c-card)', border: '1px solid var(--c-line)' }}>
          ...
          <window.Bar value={med.stock} max={med.stockMax} color={med.alert ? 'var(--c-alert)' : 'var(--c-accent)'}/>
              ...
              return (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: ok ? 'var(--c-success)' : 'var(--c-line)', display: 'grid', placeItems: 'center' }}>
                    {ok && <window.IconCheck size={14} stroke={2.5} style={{ color: '#fff' }}/>}
                  </div>
...
function ConsultaDetailScreen({ go }) {
  const consult = window.CONSULTAS[0];
  ...
  return (
    <window.Screen>
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <window.IconButton variant="plain" onClick={() => go && go('consultas')}><window.IconArrowL size={20}/></window.IconButton>
        <window.IconButton variant="soft" size={36}><window.IconShare size={18}/></window.IconButton>
      </div>
        ...
        <div style={{ background: consult.color, borderRadius: 22, padding: 22 }}>
          <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
            <div style={{ textAlign: 'center', minWidth: 60 }}>
              <window.IconCalendar size={20} style={{ color: consult.fg, margin: '0 auto 4px' }}/>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 8, fontSize: 12, color: consult.fg, opacity: .7 }}>
                <window.IconSearch size={12}/> {consult.local}
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ padding: '14px 20px 14px' }}>
        <window.ViewToggle value={tab} onChange={setTab} options={[
      ...
      {tab === 'detalhe' && (
        <div style={{ padding: '0 20px' }}>
          ...
          <div style={{ marginBottom: 16, padding: 14, borderRadius: 16, background: 'var(--c-card)', border: '1px solid var(--c-line)', display: 'flex', alignItems: 'center', gap: 12 }}>
            <window.Avatar name="Carla Almeida" color="rgb(218,235,222)" fg="var(--c-text)" size={36}/>
            <div style={{ flex: 1 }}>
            ...
            {consult.prep.map((item, i) => {
              ...
              return (
                <button key={i} onClick={() => setPrepChecked(p => ({ ...p, [i]: !p[i] }))} style={{
                  ...
                  }}>{checked && <window.IconCheck size={13} stroke={2.5}/>}</div>
          ...
          <div style={{ marginTop: 16, display: 'flex', gap: 8 }}>
            <window.Button variant="outline" full icon={<window.IconCalendar size={16}/>}>Adicionar ao calendário</window.Button>
          </div>
          <div style={{ marginTop: 10, padding: 14, borderRadius: 14, background: 'var(--c-surface)', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            <window.IconInfo size={18} style={{ color: 'var(--c-accent)', marginTop: 1, flexShrink: 0 }}/>
      ...
      {tab === 'anotacoes' && (
        <div style={{ padding: '0 20px' }}>
          ...
          />
          <window.Button variant="primary" full size="md" style={{ marginTop: 10 }}
            icon={<window.IconCheck size={16}/>}>Salvar anotação</window.Button>
        </div>
      )}
      ...
      {tab === 'historico' && (
        <div style={{ padding: '0 20px' }}>
          <div className="cu-eyebrow" style={{ padding: '0 4px 10px' }}>Consultas anteriores — {window.CONSULTAS[0].esp}</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
...
function ResumoSaudeScreen({ go }) {
  const p = window.CUIDA_DATA.PROFILES[0];
  ...
  return (
    <window.Screen>
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <window.IconButton variant="plain" onClick={() => go && go('prontuarios')}><window.IconArrowL size={20}/></window.IconButton>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button onClick={() => setEditing(e => !e)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--c-accent)', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', padding: '6px 12px', borderRadius: 999, background: editing ? 'var(--c-success)' : 'var(--c-accent-soft)' }}>
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        {/* Identity */}
        <window.SoftCard background={p.color} padding={18} radius={20}>
          <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
            <window.Avatar name={p.name} color="rgba(255,255,255,.5)" fg={p.fg} size={52}/>
            <div>
        ...
        {/* Condições */}
        <window.Card padding={16} radius={18} shadow={false}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <div style={{ fontSize: 14, fontWeight: 600 }}>Condições crônicas</div>
            {editing && <window.IconButton variant="plain" size={28} onClick={() => setConditions(c => [...c, 'Nova condição'])}><window.IconPlus size={16}/></window.IconButton>}
          </div>
          {conditions.map((c, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderTop: i > 0 ? '1px solid var(--c-line)' : 'none' }}>
              <window.Dot color="var(--c-alert)" size={8}/>
              {editing ? (
        ...
        {/* Alergias */}
        <window.Card padding={16} radius={18} shadow={false} style={{ border: '1px solid rgba(170,60,38,.2)', background: 'var(--c-alert-soft)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <window.IconAlert size={18} style={{ color: 'var(--c-alert)' }}/>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--c-alert)' }}>Alergias</div>
            </div>
            {editing && <window.IconButton variant="plain" size={24} onClick={() => setAllergies(a => [...a, 'Nova alergia'])}><window.IconPlus size={14}/></window.IconButton>}
          </div>
        ...
        <window.Card padding={16} radius={18} shadow={false}>
          ...
          {window.CUIDA_DATA.MEDS_MARIA.map(m => (
        ...
        <window.Card padding={16} radius={18} shadow={false}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              ['Plano de saúde', p.plano, <window.IconShield size={16}/>],
              ['Contato de emergência', p.emergencia, <window.IconPhone size={16}/>],
            ].map(([label, val, icon]) => (
...
function HistoricoMedicoScreen({ go }) {
  ...
  return (
    <window.Screen>
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <window.IconButton variant="plain" onClick={() => go && go('prontuarios')}><window.IconArrowL size={20}/></window.IconButton>
        <window.IconButton variant="soft" size={36} onClick={() => setShowAdd(true)}><window.IconPlus size={18}/></window.IconButton>
      </div>
      <div style={{ padding: '6px 20px 16px' }}>
      ...
      <div style={{ padding: '0 20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {group.events.map((e, ei) => (
                <div key={e.id} style={{
                  ...
                  <window.IconChevR size={16} style={{ color: 'var(--c-text-muted)', alignSelf: 'center' }}/>
                </div>
              ))}
            </div>
          </div>
        ...
        <window.Button variant="outline" full icon={<window.IconPlus size={16}/>} style={{ marginTop: 4 }} onClick={() => setShowAdd(true)}>
          Adicionar registro
        </window.Button>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
          ...
          <div style={{ position: 'relative', background: 'var(--c-bg)', borderRadius: '24px 24px 0 0', padding: '20px 20px 40px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <window.Button variant="primary" full size="lg" onClick={saveEntry}
...
function ExamesFullScreen({ go }) {
  ...
  return (
    <window.Screen>
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <window.IconButton variant="plain" onClick={() => go && go('exames')}><window.IconArrowL size={20}/></window.IconButton>
        <window.IconButton variant="soft" size={36} onClick={() => go && go('add-exame')}><window.IconPlus size={18}/></window.IconButton>
      </div>
      ...
      <div style={{ padding: '0 20px' }}>
        ...
        {(tab === 'todos' || tab === 'pendentes') && atrasados.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            <div className="cu-eyebrow" style={{ padding: '0 4px 10px', color: 'var(--c-alert)', display: 'flex', alignItems: 'center', gap: 6 }}>
              <window.Dot color="var(--c-alert)" size={7}/> Atrasados
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {atrasados.map(e => (
                <div key={e.id} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <window.Button variant="danger" size="sm">Remarcar</window.Button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {(tab === 'todos' || tab === 'realizados') && (
          <div>
            <div className="cu-eyebrow" style={{ padding: '0 4px 10px', display: 'flex', alignItems: 'center', gap: 6 }}>
              <window.Dot color="var(--c-success)" size={7}/> Realizados
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {realizados.map((e, i) => (
                <div key={e.id} style={{
                  ...
                  <div style={{ width: 42, height: 42, borderRadius: 12, background: e.color, display: 'grid', placeItems: 'center', flexShrink: 0 }}>
                    {e.status === 'laudo' ? <window.IconCheck size={18} style={{ color: 'var(--c-success)' }}/> : <window.IconClock size={18} style={{ color: 'var(--c-warn)' }}/>}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 600 }}>{e.name}</div>
...
// Fallback med row (used if micro.jsx hasn't loaded yet)
function FallbackMedRow({ med, taken, onToggle }) {
  return (
    <div onClick={onToggle} style={{
      display: 'flex', alignItems: 'center', gap: 12, padding: 12, borderRadius: 14,
      background: 'var(--c-card)', border: '1px solid var(--c-line)',
      cursor: 'pointer', opacity: taken ? .55 : 1, transition: 'opacity .2s',
    }}>
      <div style={{ width: 40, height: 40, borderRadius: 10, background: med.color,
        display: 'grid', placeItems: 'center', fontSize: 20 }}>{med.icon}</div>
      <div style={{ flex: 1 }}>
      ...
      <div style={{
        ...
        {taken && <window.IconCheck size={14} stroke={2.5}/>}
      </div>
    </div>
  );
}
...
function HomeV3Screen({ go }) {
  ...
  const profile = window.CUIDA_DATA.PROFILES.find(p => p.id === activeId);
  const meds = activeId === 'joao' ? window.CUIDA_DATA.MEDS_JOAO : window.CUIDA_DATA.MEDS_MARIA;
  ...
  return (
    <window.Screen>
      ...
      {window.AllDoneCelebration && <window.AllDoneCelebration show={allDoneShown} name={profile.name}/>}
      ...
      <div style={{ padding: '4px 20px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ position: 'relative' }}>
          <window.IconButton variant="soft" onClick={() => go && go('settings')}><window.IconBell size={20}/></window.IconButton>
      ...
      {/* Profile picker */}
      <window.ProfilePicker profiles={window.CUIDA_DATA.PROFILES} activeId={activeId} onChange={setActiveId} onAdd={() => go && go('fam')}/>
      ...
      <div style={{ padding: '16px 20px 0' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <QuickBtn icon={<window.IconCalendar size={22}/>} label="Consultas" screen="consultas"/>
          ...
          <QuickBtn icon={<window.IconShield size={22}/>} label="Prontuário" screen="prontuarios"/>
          <QuickBtn icon={<window.IconHeart size={22}/>} label="Histórico" screen="historico"/>
        </div>
      </div>
      ...
      <div style={{ padding: '18px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <window.ViewToggle value={view} onChange={setView} options={[{ value: 'timeline', label: 'Timeline' }, { value: 'list', label: 'Lista' }]}/>
      </div>
...
function TimeGroupHS({ label, emoji, meds, isTaken, toggle }) {
  return (
    <div style={{ padding: '12px 20px 0' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {meds.map(m => {
          const MedRowComp = window.AnimatedMedRow || FallbackMedRow;
          return (
            <MedRowComp
              key={m.id}
              med={m}
