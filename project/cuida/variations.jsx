...
// Three takes on each screen, exposed in design canvas

const D2 = window.CUIDA_DATA;
  ...
  return (
    <window.Screen>
      <div style={{ padding: '4px 20px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <window.IconButton variant="soft"><window.IconBell size={20}/></window.IconButton>
      </div>

      <window.ProfilePicker profiles={D2.PROFILES} activeId={'maria'} onChange={() => {}}/>

      {/* Timeline card */}
      <div style={{ padding: '4px 20px 0' }}>
        <window.SoftCard background="var(--c-surface)" padding={0} radius={24}
      ...
      <div style={{ padding: '16px 20px 0', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        <MiniAlertCard
          ...
          fg="var(--c-accent)"
          icon={<window.IconCalendar size={18}/>}
          label="Consulta"
          value="Dr. Mendes · Seg"
        />
      </div>
  ...
  return (
    <window.Screen background="var(--c-bg)">
      ...
      <div style={{ padding: '6px 20px 0', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
        <QuickAction icon={<window.IconPill size={20}/>} label="Remédios"/>
        <QuickAction icon={<window.IconShield size={20}/>} label="SOS"/>
        <QuickAction icon={<window.IconCalendar size={20}/>} label="Agenda"/>
      </div>
    </window.Screen>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <window.Avatar name={p.name} color="rgba(255,255,255,.45)" fg={p.fg} size={56}/>
      </div>
...
function EmergencyLockscreenScreen() {
  ...
  return (
    <window.Screen hasTabBar={false} background="#0a0d0e"
      ...
      <div style={{ padding: '24px 24px 0' }}>
        <div style={{
          ...
          <window.IconAlert size={18}/> SOS · MEDICAL INFO
        </div>
      </div>
      ...
      <div style={{ padding: '18px 24px 0' }}>
        <button style={{
          ...
          <window.IconPhone size={20}/> Ligar para 192 · SAMU
        </button>
        <button style={{
          ...
          <window.IconPhone size={16}/> Carla (filha) · +55 11 98123-4567
        </button>
      </div>
    </window.Screen>
  );
...
function EmergencyQRScreen() {
  const p = D2.PROFILES[0];
  return (
    <window.Screen background="var(--c-accent)" style={{ color: 'var(--c-accent-fg)' }}>
      ...
      <div style={{ padding: '14px 24px 0', display: 'grid', placeItems: 'center' }}>
        <window.FakeQR size={220} bg="var(--c-accent-fg)" fg="var(--c-accent)" radius={20}/>
        <div style={{
