// cuida/micro.jsx
// Enhanced versions of TabBar, MedRow and Screen with animations.
// Loaded AFTER ui.jsx — overrides window.TabBar and augments MedRow/Screen.

const { useState: useMicro, useEffect: useEffectMicro, useRef: useRefMicro, useCallback: useCB } = React;

// ════════════════════════════════════════════════════════════
// ANIMATED TAB BAR
// Sliding pill indicator + entrance animation per icon
// ════════════════════════════════════════════════════════════
function AnimatedTabBar({ active, onChange }) {
  const tabs = [
    { id: 'home',  label: 'Início',    Icon: window.IconHome },
    { id: 'meds',  label: 'Remédios',  Icon: window.IconPill },
    { id: 'stock', label: 'Estoque',   Icon: window.IconBox },
    { id: 'emerg', label: 'SOS',       Icon: window.IconShield },
    { id: 'fam',   label: 'Família',   Icon: window.IconUsers },
  ];
...
// ════════════════════════════════════════════════════════════
// ANIMATED MED ROW
// Bounce on check + row flash + success burst
// ════════════════════════════════════════════════════════════
      ...
      >
        {taken && (
          <div className="cu-check-enter">
            <window.IconCheck size={16} stroke={2.5}/>
          </div>
        )}
      </div>
    </div>
...
// Shows when all meds for a day are confirmed
// ════════════════════════════════════════════════════════════
...
// Wraps screen changes with slide transitions
// ════════════════════════════════════════════════════════════
...
function AnimatedScreenRouter({ screen, children }) {
  ...
  const [isTransitioning, setIsTransitioning] = useMicro(false);
  ...
  useEffectMicro(() => {
    ...
    // Determine direction: going back = from left, going forward = from right
...
// ════════════════════════════════════════════════════════════
// Export — override window globals
// ════════════════════════════════════════════════════════════
Object.assign(window, {
  // Override TabBar with animated version
  ...
  // Override ViewToggle with animated version
  ViewToggle: AnimatedViewToggle,
});
