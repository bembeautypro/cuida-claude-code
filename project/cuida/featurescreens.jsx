...
const { useState: useFeat, useRef: useRefFeat } = React;
  ...
  return (
    <window.Screen hasTabBar={false}>
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <window.IconButton variant="soft" size={36} onClick={() => go && go('add-consulta')}><window.IconPlus size={18}/></window.IconButton>
      </div>
        ...
        {selectedEvents.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '28px 20px', color: 'var(--c-text-muted)', fontSize: 13 }}>
            ...
            <div style={{ marginTop: 16 }}>
              <window.Button variant="outline" size="sm" onClick={() => go && go('add-consulta')} icon={<window.IconPlus size={14}/>}>
                Agendar consulta
              </window.Button>
            </div>
          </div>
            ...
            <div key={ev.id} style={{ borderRadius: 20, overflow: 'hidden', marginBottom: 12, border: '1px solid var(--c-line)', background: 'var(--c-card)', boxShadow: 'var(--shadow-sm)' }}>
              ...
              <div style={{ padding: '12px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  {resp ? (
                    <>
                      <window.Avatar name={resp} color={ev.color} fg="var(--c-text)" size={28}/>
              ...
              <div style={{ padding: '0 18px 14px', display: 'flex', gap: 8 }}>
                ...
                <window.Button variant="soft" size="sm" style={{ flex: 1 }} onClick={() => go && go('acompanhante')}>Acompanhante</window.Button>
              </div>
            </div>
          );
        })}
          ...
          <div style={{ position: 'relative', background: 'var(--c-bg)', borderRadius: '24px 24px 0 0', padding: '20px 20px 36px' }}>
            ...
            {FAMILY_MEMBERS.map(f => (
              <button key={f.id} onClick={() => assignResp(showRespModal, f.name)} style={{
                ...
                <window.Avatar name={f.name} color={f.color} fg="var(--c-text)" size={40}/>
...
function AcompanhanteScreen({ go }) {
  ...
  return (
    <window.Screen hasTabBar={false}>
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <window.IconButton variant="plain" onClick={() => go && go('consultas-cal')}><window.IconArrowL size={20}/></window.IconButton>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {members.map(f => (
            <div key={f.id} style={{
              ...
              <window.Avatar name={f.name} color={f.color} fg="var(--c-text)" size={44}/>
        ...
        {members.some(m => m.status === 'pendente') && (
          <div style={{ marginTop: 14, padding: 14, borderRadius: 16, background: 'var(--c-surface)', border: '1px solid var(--c-line)' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <window.Button variant="success" size="sm" style={{ flex: 1 }} onClick={() => setMembers(prev => prev.map(m => m.id === 'rafael' ? { ...m, status: 'aceito' } : m))}>
                ✓ Aceitar
              </window.Button>
              <window.Button variant="danger" size="sm" style={{ flex: 1 }} onClick={() => setMembers(prev => prev.map(m => m.id === 'rafael' ? { ...m, status: 'recusado' } : m))}>
                Recusar
              </window.Button>
            </div>
          </div>
      ...
      <div style={{ padding: '22px 20px 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
        <window.Button variant="primary" full size="lg" icon={<window.IconWhatsApp size={18}/>} onClick={sendInvites}>
          {sent ? '✓ Convites enviados!' : 'Enviar convites pelo WhatsApp'}
        </window.Button>
        <window.Button variant="outline" full size="md" icon={<window.IconShare size={16}/>}>
  ...
  return (
    <window.Screen hasTabBar={false}>
      ...
      <div style={{ padding: '4px 20px 10px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: '1px solid var(--c-line)' }}>
        <window.IconButton variant="plain" onClick={() => go && go('fam')}><window.IconArrowL size={20}/></window.IconButton>
            ...
            <div key={msg.id} style={{ display: 'flex', flexDirection: msg.mine ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: 8 }}>
              ...
              {!msg.mine && (
                <div style={{ width: 28, flexShrink: 0, marginBottom: 2 }}>
                  {showAvatar && <window.Avatar name={msg.from} color={AVATARS[msg.from] || 'var(--c-surface)'} fg="var(--c-text)" size={28}/>}
                </div>
              )}
      ...
      <div style={{ padding: '6px 16px 12px', display: 'flex', alignItems: 'center', gap: 10, borderTop: '1px solid var(--c-line)', background: 'var(--c-bg)' }}>
        <window.IconButton variant="soft" size={36}><window.IconCamera size={18}/></window.IconButton>
        <input
          value={input}
