// Cuida — Main 7 screens for the interactive prototype
// All assume window.CUIDA_DATA loaded

const { useState: useStateScreens } = React;
const D = window.CUIDA_DATA;

// ════════════════════════════════════════════════════════════
// PROFILE PICKER (used in Home, Medicamentos)
// ════════════════════════════════════════════════════════════
        ...
        return (
          <button key={p.id} onClick={() => onChange(p.id)} style={{
            ...
            <div style={{
              ...
              <window.Avatar name={p.name} color={p.color} fg={p.fg} size={54}/>
              {p.medsAlert > 0 && (
                <span style={{
...
// ════════════════════════════════════════════════════════════
// 1. HOME — Timeline (default) + Lista toggle
// ════════════════════════════════════════════════════════════
  ...
  return (
    <window.Screen>
      ...
      <div style={{ padding: '4px 20px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <window.IconButton variant="soft" onClick={() => go && go('settings')}>
          <window.IconBell size={20}/>
          <span style={{
      ...
      <div style={{ padding: '6px 20px 0' }}>
        <window.SoftCard background={profile.color} padding={18} radius={22}
...
// ── Timeline body — curved day visualization ──
function HomeTimelineBody({ meds, morning, afternoon, night, isTaken, totalTaken, nextDose }) {
  // current time marker — 14:32 (between morning and afternoon)
  return (
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', height: 130, padding: '14px 0 0' }}>
          <svg width="100%" height="100%" viewBox="0 0 320 120" preserveAspectRatio="none" style={{ display: 'block', overflow: 'visible' }}>
            ...
            {/* time marker (14:32 ≈ x: 200) — current */}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {D.PROFILES.slice(1).map(p2 => (
          <div key={p2.id} style={{
            ...
            <window.Avatar name={p2.name} color={p2.color} fg={p2.fg} size={36}/>
            <div style={{ flex: 1 }}>
            ...
            <window.Dot color={p2.medsAlert > 0 ? 'var(--c-alert)' : 'var(--c-success)'} size={8}/>
          </div>
        ))}
      </div>
    </div>
...
function TimelineDose({ label, meds, isTaken }) {
  ...
  const someDone = meds.some(isTaken);
  return (
    <div style={{
...
// ── List body — grouped medication list (original V1) ──
  ...
  return (
    <div onClick={onToggle} style={{
      ...
      <div style={{
        ...
        {taken && <window.IconCheck size={16} stroke={2.5}/>}
      </div>
    </div>
  );
}

// ⚠ MedsScreen removida — substituída por MedsV2Screen em newscreens.jsx
...
function StockScreen({ go }) {
  ...
  return (
    <window.Screen>
      <window.ScreenHeader title="Estoque"/>

      <div style={{ padding: '0 20px 14px', display: 'flex', gap: 8 }}>
        <window.Pill active={filter === 'all'} onClick={() => setFilter('all')} color="var(--c-accent)" fg="var(--c-accent-fg)">Tudo · {D.STOCK_ALL.length}</window.Pill>
        <window.Pill active={filter === 'critical'} onClick={() => setFilter('critical')} color="var(--c-alert)" fg="#fff">Crítico · {critical.length}</window.Pill>
      </div>

      {/* Shopping list summary card */}
      {critical.length > 0 && (
        <div style={{ padding: '0 20px 14px' }}>
          <window.SoftCard background="var(--c-accent)" padding={18} radius={20}
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <window.Button variant="soft" size="sm" icon={<window.IconWhatsApp size={16}/>} style={{ background: 'var(--c-accent-fg)', color: 'var(--c-accent)', flex: 1 }}>WhatsApp</window.Button>
              <window.Button variant="ghost" size="sm" icon={<window.IconShare size={16}/>} style={{ color: 'var(--c-accent-fg)', background: 'rgba(255,255,255,.1)', flex: 1 }}>Compartilhar</window.Button>
            </div>
          </window.SoftCard>
        </div>
      )}
            ...
            <div key={s.id} style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <window.Bar value={s.stock} max={s.max} color={s.critical ? 'var(--c-alert)' : 'var(--c-accent)'} bg="rgba(38,37,37,.06)"/>
...
// ════════════════════════════════════════════════════════════
// 4. EMERGÊNCIA — toggle Lock screen (default) + Carteirinha
// ════════════════════════════════════════════════════════════
function EmergencyScreen({ go, initialView = 'lockscreen' }) {
  ...
  return (
    <window.Screen background="var(--c-bg)">
      <window.ScreenHeader title="Emergência" eyebrow="Card de SOS"/>
      ...
      <div style={{ padding: '18px 20px 0', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <window.Button variant="primary" icon={<window.IconShare size={18}/>}>Compartilhar</window.Button>
        <window.Button variant="soft" icon={<window.IconPrint size={18}/>}>Imprimir</window.Button>
        <window.Button variant="outline" icon={<window.IconLink size={18}/>} style={{ gridColumn: '1 / -1' }}>Copiar link público</window.Button>
      </div>

      {/* Hint */}
      <div style={{ padding: '20px 20px 0' }}>
        <window.SoftCard background="var(--c-surface)" padding={16} radius={16}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            <window.IconInfo size={20} style={{ color: 'var(--c-accent)', marginTop: 2, flexShrink: 0 }}/>
            <div>
...
// ── Lock-screen preview (compact, lives inside the Emergência tab) ──
    ...
    <div>
      <div style={{
        ...
        <div style={{
          ...
          <window.IconAlert size={14}/> SOS · Medical info
        </div>
      ...
      {/* Present full-screen CTA */}
      <window.Button variant="dark" full size="lg" style={{ marginTop: 12 }} icon={<window.IconShield size={18}/>}>
...
// Carteirinha-style emergency card (default variation)
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18, position: 'relative' }}>
        ...
        <window.BrandMark size={28} style={{ color: 'var(--c-accent-fg)' }}/>
      </div>
...
function FamilyScreen({ go }) {
  return (
    <window.Screen>
      <window.ScreenHeader title="Família" action={
        <div style={{ display: 'flex', gap: 8 }}>
          <window.IconButton variant="soft" size={42} onClick={() => go && go('mensagens')}>
            <window.IconChat size={20}/>
          </window.IconButton>
          <window.IconButton variant="soft" size={42} onClick={() => go && go('feed')}>
            <window.IconBell size={20}/>
          </window.IconButton>
          <window.IconButton variant="accent" size={42}>
      ...
      <div style={{ padding: '0 20px 6px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {D.PROFILES.map(p => (
            <window.Card key={p.id} padding={16} radius={18} shadow={false}
              style={{ display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer' }}>
              <window.Avatar name={p.name} color={p.color} fg={p.fg} size={48}/>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 16, fontWeight: 600 }}>{p.name}</div>
                <div style={{ fontSize: 12, color: 'var(--c-text-soft)' }}>{p.relation} · {p.medsToday} remédios/dia</div>
              </div>
              {p.medsAlert > 0 && <window.Dot color="var(--c-alert)" size={10}/>}
      ...
      <div style={{ padding: '18px 20px 0' }}>
        ...
        <button style={{
          ...
          <window.IconWhatsApp size={22} style={{ color: 'var(--c-accent)' }}/>
...
function FeedScreen({ go }) {
  ...
  return (
    <window.Screen>
      <window.ScreenHeader title="Feed" eyebrow="Atividade da família"/>
...
function SettingsScreen({ go }) {
  return (
    <window.Screen>
      <window.ScreenHeader title="Configurações"/>

      {/* Account */}
      <div style={{ padding: '0 20px 14px' }}>
        <window.Card padding={18} radius={20} shadow={false} onClick={() => go && go('edit-profile')}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <window.Avatar name="Carla Almeida" color="rgb(218, 235, 222)" size={54}/>
            <div style={{ flex: 1 }}>
          ...
          <div style={{
            ...
            <window.Button variant="primary" size="sm" onClick={() => go && go('plano')}>Gerenciar</window.Button>
          </div>
        </window.Card>
      </div>

      <SettingsGroup title="Cuidado">
        <SettingsRow icon={<window.IconBell size={20}/>} label="Notificações" detail="Push + WhatsApp" onClick={() => go && go('notificacoes')}/>
        <SettingsRow icon={<window.IconUsers size={20}/>} label="Família" detail="3 pessoas"/>
        <SettingsRow icon={<window.IconCalendar size={20}/>} label="Lembretes" detail="Padrão por horário"/>
      </SettingsGroup>

      <SettingsGroup title="Privacidade">
        <SettingsRow icon={<window.IconLock size={20}/>} label="Privacidade e segurança"/>
        <SettingsRow icon={<window.IconShield size={20}/>} label="Card de emergência público" detail="Ativo"/>
        <SettingsRow icon={<window.IconShare size={20}/>} label="Compartilhamentos" detail="2 ativos"/>
      </SettingsGroup>

      <SettingsGroup title="Sobre">
        <SettingsRow icon={<window.IconInfo size={20}/>} label="Ajuda e suporte"/>
        <SettingsRow icon={<window.IconHeart size={20}/>} label="Indique cuida" detail="Ganhe 1 mês" onClick={() => go && go('indicar')}/>
      </SettingsGroup>
...
// MedsScreen foi substituída por MedsV2Screen (newscreens.jsx) — não exportar
