  ...
  return (
    <window.Screen>
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <window.IconButton variant="soft" size={36}><window.IconPlus size={18}/></window.IconButton>
      </div>
      ...
      {atrasadas.length > 0 && (
        <div style={{ padding: '10px 20px 0' }}>
          <div style={{
            ...
            <window.IconAlert size={18} style={{ color: 'var(--c-alert)', flexShrink: 0 }}/>
            <div style={{ flex: 1 }}>
      ...
      <div style={{ padding: '20px 20px 0' }}>
        <window.Button variant="outline" full size="md" icon={
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
            <path d="m18 2 4 4-9.4 9.4-4-4L18 2zM5 17l-3 3M10 3 7 6M21 14l-3 3"/>
          </svg>
        }>Registrar vacina aplicada</window.Button>
      </div>
    </window.Screen>
  );
}
  ...
  return (
    <window.Screen hasTabBar={false}>
      <div style={{ padding: '20px 28px 24px', display: 'flex', flexDirection: 'column', height: '100%' }}>
        <window.IconButton variant="plain" size={36} onClick={() => go && go('home')}>
        ...
        <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <window.Button variant="primary" size="lg" full onClick={handleLogin} style={{ opacity: loading ? .7 : 1 }}>
    ...
    <window.Screen hasTabBar={false}>
      ...
      <div style={{ display: 'grid', placeItems: 'center', padding: '20px 0 16px' }}>
        <div style={{ position: 'relative' }}>
          <window.Avatar name={p.name} color={p.color} fg={p.fg} size={80}/>
          <button style={{
          ...
          }}><window.IconCamera size={14}/></button>
        </div>
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <window.Button variant="primary" full size="lg" onClick={handleSave}
...
function EPRow({ label, value, detail }) {
  return (
    <div style={{
      ...
      <window.IconChevR size={14} style={{ color: 'var(--c-text-muted)', flexShrink: 0 }}/>
    </div>
  );
}
...
function AgendarConsultaScreen({ go }) {
  ...
  const ESPS = [
    { id:'cardio', label:'Cardiologia',    icon: <window.IconHeart size={18}/>,    color:'rgb(252,224,213)' },
    { id:'endo',   label:'Endocrinologia', icon: <window.IconDroplet size={18}/>,  color:'rgb(254,220,195)' },
    ...
    { id:'ortho',  label:'Ortopedia',      icon: <window.IconShield size={18}/>,   color:'rgb(218,235,222)' },
    ...
    { id:'geral',  label:'Clínico geral',  icon: <window.IconInfo size={18}/>,     color:'rgb(230,230,218)' },
  ];
  ...
  return (
    <window.Screen hasTabBar={false}>
      <div style={{ padding: '4px 20px 0', display: 'flex', alignItems: 'center', gap: 14 }}>
        <window.IconButton variant="plain" onClick={() => step > 1 ? setStep(s => s - 1) : go && go('consultas')}>
          <window.IconArrowL size={20}/>
        </window.IconButton>
      ...
      {step === 2 && (
        <div style={{ padding: '16px 20px 0' }}>
          ...
          <window.Button variant="primary" full size="lg"
      ...
      {step === 3 && (
        <div style={{ padding: '16px 20px 0' }}>
          ...
          <window.Button variant="primary" full size="lg" onClick={handleConfirm}
            style={{ background: confirming ? 'var(--c-success)' : undefined }}>
            {confirming ? '✓ Consulta agendada!' : 'Confirmar agendamento'}
          </window.Button>
          <window.Button variant="ghost" full size="md" style={{ marginTop: 10 }} onClick={() => go && go('consultas')}>
            Cancelar
          </window.Button>
        </div>
      )}
      ...
      <div style={{ padding: '16px 20px 0', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: 'var(--c-card)', borderRadius: 18, overflow: 'hidden', border: '1px solid var(--c-line)' }}>
            {EXAMES_LISTA.map(e => (
              <button key={e} onClick={() => setSelectedExame(e)} style={{
                ...
                {selectedExame === e && <window.IconCheck size={16} style={{ color: 'var(--c-accent)' }}/>}
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{
            ...
            <window.IconCalendar size={18} style={{ color: 'var(--c-accent)' }}/>
            <span style={{ fontSize: 15, fontWeight: 500 }}>22 de junho · 08:00</span>
            <window.IconChevR size={16} style={{ color: 'var(--c-text-muted)', marginLeft: 'auto' }}/>
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
            <button onClick={() => go && go('laudo-ocr')} style={{
              ...
              <window.IconCamera size={16}/> Foto com IA
            </button>
            <button onClick={() => setHasDoc(d => !d)} style={{
              ...
              <window.IconShare size={16}/> Upload
            </button>
          </div>
          <button onClick={() => setHasDoc(d => !d)} style={{
            width: '100%', padding: '14px 16px', borderRadius: 16,
    ...
    <window.Screen hasTabBar={false}>
      ...
      <div style={{ padding: '16px 20px 0', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div>
          ...
          <div style={{ background: 'var(--c-card)', borderRadius: 18, overflow: 'hidden', border: '1px solid var(--c-line)' }}>
            <NToggleRow icon={<window.IconBell size={18}/>} label="Notificações push" sub="Alertas direto no celular" value={push} onChange={setPush}/>
            <NToggleRow icon={<window.IconWhatsApp size={18}/>} label="WhatsApp" sub="Mensagens para lembrar doses" value={whatsapp} onChange={setWhatsapp}/>
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ background: 'var(--c-card)', borderRadius: 18, overflow: 'hidden', border: '1px solid var(--c-line)' }}>
            <NToggleRow icon={<window.IconCalendar size={18}/>} label="Lembretes de consulta" sub="1 dia e 1 hora antes" value={consultaAlert} onChange={setConsultaAlert}/>
          </div>
        </div>
